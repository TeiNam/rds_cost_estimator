# ANALYTICS_PLATFORM Oracle 마이그레이션 전략 리포트
...
**소스 데이터베이스**: ANALYTICS_PLATFORM (Oracle 19.0.0.0.0)
...
| 항목 | 값 | 설명 |
|------|-----|------|
| 전체 DB 크기 | 520.0 GB | 중간 규모로 일반적인 마이그레이션 절차를 적용합니다 |
...
| **총 PL/SQL 오브젝트 수** | **335** | - | 변환 대상 프로그램 오브젝트 총합 |
...
| 항목 | PostgreSQL | MySQL |
|------|------------|-------|
| 평균 복잡도 | 3.64 (낮음) | 4.36 (중간) |
| 최대 복잡도 | 8.64 (매우 높음) | 8.64 (매우 높음) |
| 고난이도 | 2개 / 13개 (≥5.0) | 1개 / 6개 (≥7.0) |

> **분석**: MySQL 타겟이 PostgreSQL보다 복잡도가 **0.72** 높습니다. PostgreSQL이 Oracle 호환성이 더 좋아 변환이 용이합니다.
...
|------|-----|
| PL/SQL 오브젝트 수 | 335개 |
| PL/SQL 총 라인 수 | 85,000줄 |

### 예상 작업 시간 (4인 팀 기준)

| 작업 방식 | 예상 기간 | 비고 |
|----------|----------|------|
| AI 활용 (4인) | 13~21일 (0.6~1.0개월) | 오브젝트당 4시간 × 30~50% |
...
|----------|------|------|--------|----------|
| PL/SQL 평균 복잡도 | <3.0: 0점, 3.0~5.0: 1점, 5.0~7.0: 2점, ≥7.0: 3점 | 0~3 | 3.64 (중간 (3.0~5.0)) | 1점 |
| PL/SQL 코드량 | <10K: 0점, 10K~50K: 1점, 50K~100K: 2점, ≥100K: 3점 | 0~3 | 85,000줄 (많음 (50K~100K)) | 2점 |
| 고난이도 비율 (≥5.0) | <10%: 0점, 10~30%: 1점, ≥30%: 2점 | 0~2 | 2/13개 (15.4%) | 1점 |
| 고난이도 절대 개수 | <20: 0점, 20~50: 1점, 50~100: 2점, ≥100: 3점 | 0~3 | 2개 (적음 (<20개)) | 0점 |
| 패키지 개수 | <50: 0점, 50~100: 1점, ≥100: 2점 | 0~2 | 35개 (적음 (<50)) | 0점 |
| **합계** | - | **0~13** | - | **4점** |
...
|------|-----|
| 변환 대상 오브젝트 | 335개 |
| 예상 기간 (4인, AI 활용) | 13~21일 |
| 예상 기간 (월 환산) | 0.6~1.0개월 |

## 판정 요약

- 🟡 PL/SQL 평균 복잡도가 중간 수준
- ⚠️ PL/SQL 코드량이 많음 (85,000줄)
- ⚠️ 고난이도 오브젝트 2개 존재 (복잡도 ≥5.0)
...
**추천 근거**: 예상 피크 메모리(65.0GB)를 고려하여 **db.r6i.4xlarge** (vCPU 16, 메모리 128GB)를 추천합니다. Aurora PostgreSQL의 자동 스케일링 기능으로 피크 시 추가 확장이 가능합니다.
...
## 위험 1: UTL_FILE 1회 사용 - 대체 구현 필요

**카테고리**: technical

**심각도**: high

**완화 방안**: 애플리케이션 레벨 파일 처리로 이관

## 위험 2: PL/SQL 13개를 PL/pgSQL로 변환 필요 (일부 기능 미지원)
...
## 위험 3: DBMS_SQL 1회 사용 - 대체 구현 필요

**카테고리**: technical

**심각도**: medium

**완화 방안**: 동적 SQL 재작성 (EXECUTE)

## 위험 4: 고복잡도 오브젝트 비율(15.4%)이 높아 테스트 기간 증가 예상
...
## 위험 5: 중첩 테이블(NESTED TABLE) 4회 사용 - 변환 필요
...
| 평균 PL/SQL 복잡도 | 3.64 | 0~10 척도 |
| 고난이도 SQL | 0개 | ≥5.0 기준 |
| 고난이도 PL/SQL | 2개 | ≥5.0 기준 |
| BULK 연산 | 0개 | 성능 영향 |
| 평균 CPU | 33.2% | AWR 기준 |
| 평균 I/O | 71.0 IOPS | AWR 기준 |
| 평균 메모리 | 54.2 GB | AWR 기준 |
